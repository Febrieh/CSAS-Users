...
function extractAspectsData(comments, predictedSentiment, selectedSentiment) {
    ...
    Object.keys(aspects.positive).concat(Object.keys(aspects.negative)).forEach(aspect => {
        sentimentCounts.positive[aspect] = 0;
        sentimentCounts.negative[aspect] = 0;
        sentimentCounts.neutral[aspect] = 0;
    });
    ...
    // Remove aspects with a count of 0
    Object.keys(sentimentCounts.positive).forEach(aspect => {
        if (sentimentCounts.positive[aspect] === 0) {
            delete sentimentCounts.positive[aspect];
        }
    });

    Object.keys(sentimentCounts.negative).forEach(aspect => {
        if (sentimentCounts.negative[aspect] === 0) {
            delete sentimentCounts.negative[aspect];
        }
    });
...
// Function to render positive aspects bar chart
function renderPositiveAspectsChart(sentimentCounts) {
    const positiveKeywords = sentimentCounts.positive;
    const keywords = Object.keys(positiveKeywords);
    const counts = Object.values(positiveKeywords);

    if (keywords.length === 0) {
        console.log("No positive aspects to render!");
        return;
    }
...
// Function to render negative aspects bar chart
function renderNegativeAspectsChart(sentimentCounts) {
    let negativeKeywords = sentimentCounts.negative;

    // Remove aspects with a value of 0 and round values to whole numbers
    negativeKeywords = Object.fromEntries(
        Object.entries(negativeKeywords)
            .map(([aspect, count]) => [aspect, Math.round(count)]) // Round values
            .filter(([_, count]) => count > 0) // Remove aspects with 0 value
    );

    const keywords = Object.keys(negativeKeywords);
    const counts = Object.values(negativeKeywords);

    if (keywords.length === 0) {
        console.log("No negative aspects to render!");
        return;
    }
            ...
            scales: {
                x: {
                    ...
                    ticks: {
                        ...
                        precision: 0 // Prevent decimals from showing
                    }
                },
                y: {
                    title: {
...
// Function to render sentiment breakdown pie chart
...
function extractSentimentData(resortData) {
    ...
    for (const key in csvData) {
        ...
        if (data["Predicted Sentiment"] === "Positive") {
            positiveReviews++;
        } else if (data["Predicted Sentiment"] === "Neutral") {
            neutralReviews++;
        } else if (data["Predicted Sentiment"] === "Negative") {
            negativeReviews++;
        }
        totalReviews++;
    }

    return { comments, predictedSentiment, positiveReviews, neutralReviews, negativeReviews, totalReviews };
}
...
async function fetchReviewsData(resortId) {
    try {
        ...
        if (snapshot.exists()) {
            ...
            const { comments, predictedSentiment, positiveReviews, neutralReviews, negativeReviews, totalReviews } = extractSentimentData(resortData);
            ...
            // Extract aspect data using the updated approach
            const sentimentCounts = extractAspectsData(comments, predictedSentiment, "All");

            // Render aspect charts
            renderPositiveAspectsChart(sentimentCounts);
            renderNegativeAspectsChart(sentimentCounts);
        }
    } catch (error) {
