...
function displayReviews(reviews) {
    ...
    Object.keys(reviews).forEach(resortId => {
        const resortData = reviews[resortId];

        // Check if 'details' exist before accessing it
        if (!resortData || !resortData.details) {
            console.warn(`Skipping resort ID: ${resortId} due to missing details.`);
            return;
        }

        const resortDetails = resortData.details;
        console.log("Resort ID:", resortId);
        console.log("Resort Details:", resortDetails);

        const totalRating = Object.keys(resortData)
            .filter(reviewKey => reviewKey.startsWith("review_"))
            .reduce((acc, reviewKey) => {
                const review = resortData[reviewKey];
                return review?.rating != null && !isNaN(review.rating) ? acc + review.rating : acc;
            }, 0);
        ...
        if (reviewCount > 0) {
            const maxRating = 5 * reviewCount;
            const percentage = (totalRating / maxRating) * 100;

            resortRatings.push({
                resortId,
                name: resortDetails.name || "Unknown Resort",
                percentage,
                reviewCount,
                totalRating
            });
    ...
    displayTopPerformingResorts(resortRatings.slice(0, 1));
    displayLowPerformingResorts(resortRatings.slice(-1));
}
